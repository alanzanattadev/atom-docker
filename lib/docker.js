...
import {createComposeFileSelectedAction} from './redux/actions/services';
      ...
      if (grammarName != "YAML") {
        ...
        composeFile.read().then((content) => {
          try {
            ...
            let services = Object.keys(yamlContent.version == '2' ? yamlContent.services : yamlContent).map((key) => {return {name: key}});
            store.dispatch(createComposeFileSelectedAction(composeFilePath, services));
